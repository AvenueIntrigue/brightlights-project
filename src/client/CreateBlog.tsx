...
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const hasPermission = publicMetadata.permissions?.includes('create:blog_post');

    if (!hasPermission) {
      console.error('User does not have permission to create blog posts.');
      return;
    }
    ...
    if (response.ok) {
      ...
      console.error('Error creating blog post', response.statusText, errorDetails);
    }
  } catch (error: any) {
    ...
    <div className="create-blog-container mx-auto">
      <form className="create-blog" onSubmit={handleSubmit}>
        <div className="form-container">
          <h1 className="form-box-text">Create Blog Post</h1>
        </div>
